...
const objLat = (obj) => {
    ...
    return ("my name is " + obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1) +
    ...
    for (let i = 0; i < arr.length; i++) {
        if(arr[i].yearsOfExperience <= 1){
            ...
            if (arr[i]['firstName'] != null & arr[i]['lastName'] != null) {
                ...
                arr2.push({"fullName":arr[i].lastName,"tech":arr[i].tech})
            }
            else {
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
        ...
        // Iterate over each class in the grade
        for (let j = 0; j < grade.classes.length; j++) {
          const currentClass = grade.classes[j];
          let sum = 0;
      
          // Calculate the sum of all class scores
          for (let k = 0; k < currentClass.classScores.length; k++) {
            sum += currentClass.classScores[k];
          }
      
          // Calculate the average score and update the class object
          const avg = sum / currentClass.classScores.length;
          currentClass.avg = Math.floor(avg);
        }
      }
      return data;
    // write your code here
