...
const recursionPattern = (int1, int2) => {
    ...
    function recursion(num1, num2) {
        arr.push(num1)
        if ((num1 - num2) > 0) {
            return recursion((num1 - num2), num2)
        }
        else {
            num1 = num1 - num2;
            while (num1 <= arr[0]) {
...
const filterLinks = (str) => {
    var match = str.match(/href="([^"]*)"/);

    if (match && match.length > 1) {
        var link = match[1];
        var regex = /\.com$|\.org$|\.net$/i; 
        if (regex.test(link)) {
           
            var link2 = link.replace(/^https?:\/\//i, "");
            return link2;
        }
    }
    // write your code here
...
const isPalindrome = (str) => {
    let result = (str.replace(/[\s\p{P}]+/gu, '')).toLowerCase();
